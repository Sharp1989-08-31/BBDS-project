...
# Using "CODE COLLAPSE" keyboard shortcuts for RStudio is recommended:
#    "ALT + O" to collapse all (do this one right away!)
#    "SHIFT + ALT + O" to uncollapse all
#
#    "ALT + L" to collapse single section
#    "SHIFT + ALT + L" to uncollapse single section
...
species<-"BLGU"   # set as "TBMU" or "BLGU"
looseFilter<-0.75
stringentFilter<-0.9
showBatch<-FALSE
...
#### LOAD CHEMICAL DATA ####
chemRaw <- read.csv(paste0("../Data/",chemFile), row.names = 'Chemical')
group <- read.csv('../Data/Chemical grouping.csv')
mdl <- read.csv('../Data/mdl.csv',row.names='chemical')
...
# load pcr data
pcrRaw <- read.csv(paste0("../Data/", pcrFile), row.names = 'Gene')
...
par(mfrow=c(3,3), mar=c(4,3,1.5,1))
for(i in order(plsV2$cvres$slope[,v2Comps[1]],decreasing=TRUE)[1:9]){
  dataRange<-c(
